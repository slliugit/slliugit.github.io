...
def __noise_sample_generator(info_file,fs, length_seq, split):
    head=info_file
    val_samples=[]
    val_samples.extend(glob.glob(os.path.join(head,"white.wav")))
    
    while True:
        r = list(range(len(val_samples)))
        
        if split!="test":
            random.shuffle(r)
        for i in r:
            loaded_data, Fs=sf.read(val_samples[i])
...
import scipy.signal
def generate_test_data(path_music, path_noises,num_samples=-1, fs=44100, seg_len_s=20):
    ...
    for path in path_music:
        ...
        #load data clean files
        jj=0
        for i,file in enumerate(tqdm(train_samples)):  #add [1:5] for testing
            data_clean, samplerate = sf.read(file)
            if samplerate!=fs: 
                print("!!!!WRONG SAMPLE RATe!!!")
                data_clean=scipy.signal.resample(data_clean, int((44100  / samplerate )*len(data_clean))+1) 
            #Stereo to mono
            ...
            for snr in SNRs:
                ...
                # sf.write('/home/liushulin/workspace2/denoising3/clean/%03d.wav'%i,segment.astype('float32'),fs)
                sf.write('./clean/%03d.wav' % i, segment.astype('float32'), fs)
          
                sf.write('./noise/%03d.wav'%i,summed.astype('float32'),fs)
...
def generator_train(path_music, path_noises,split, fs=44100, seg_len_s=5, extend=True, stereo=False):
    ...
    noise_generator=__noise_sample_generator(path_noises.decode("utf-8"),fs, seg_len,  split.decode("utf-8")) #this will take care of everything
    #load data clean files
    while True:
...
if __name__ == '__main__':
  ...
  segments_noisy, segments_clean=generate_test_data(['dataset/test_yanchang'],
  'dataset/noise441')
